...
app.post('/api/ds-composite', rateLimiter, async (req, res) => {
  dsCors(req, res);
  try {
    const { photoUrl, maskDataUrl, segmentLabel, segmentPosition, product, photoSize, quality, paintColor, mode, userPrompt, styleName, roomDescription, pano } = req.body || {};
    ...
    const data = await compositeProduct({ photoUrl, maskDataUrl, segmentLabel, segmentPosition, product, photoSize, quality, paintColor, mode, userPrompt, styleName, roomDescription, pano });
    res.json(data);
  } catch (error) {
